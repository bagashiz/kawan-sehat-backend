...
// accountIllnessHistoryResponse holds the response data for the account's illness history object.
type accountIllnessHistoryResponse struct {
...
// ListAccountIllnessHistories is the handler for the account's illness histories retrieval route.
func (h *Handler) ListAccountIllnessHistories() APIFunc {
	return func(w http.ResponseWriter, r *http.Request) error {
		illnessHistories, err := h.userSvc.ListAccountIllnessHistories(r.Context())
		if err != nil {
			return handleUserError(err)
		}
		var res []*accountIllnessHistoryResponse
		for _, history := range illnessHistories {
			res = append(res, &accountIllnessHistoryResponse{
